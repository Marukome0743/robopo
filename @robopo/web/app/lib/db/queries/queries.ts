...
function firstTimeSubquery(
  ...
  return sql`
    (
      SELECT TO_CHAR(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo', 'YYYY-MM-DD HH24:MI:SS') FROM (
        SELECT
...
function firstAttemptTimeSubquery(
  ...
  return sql`
    (
      SELECT TO_CHAR(MIN(created_at) AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo', 'YYYY-MM-DD HH24:MI:SS')
      FROM challenge
      WHERE
        player_id = ${playerIdRef}
        AND competition_id = ${competitionId}
...
function lastAttemptTimeSubquery(
  ...
  return sql`
    (
      SELECT TO_CHAR(MAX(created_at) AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo', 'YYYY-MM-DD HH24:MI:SS')
      FROM challenge
      WHERE
        player_id = ${playerIdRef}
        AND competition_id = ${competitionId}

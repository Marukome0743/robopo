...
function firstCountSubquery(
  playerIdRef: ReturnType<typeof sql>,
...
// SQL helper: total challenge count (counting retries as 2) for specific courses
function challengeCountExpr(courseId: number) {
  return sql`SUM(CASE
...
export async function getCourseSummary(
  ...
  const result = await db
    .select({
      ...
      challengeCount: challengeCountExpr(courseId).as("challengeCount"),
    })
    .from(player)

...
export function ChallengeTab({
  ...
  // Apply localStorage values after hydration
  useEffect(() => {
    if (!isLoaded) {
      return
    }

    if (!singleCompetition && stored.competitionId) {
      const isValid = activeCompetitions.some(
        (c) => c.id === stored.competitionId,
      )
      if (isValid) {
    ...
    if (!loggedInJudgeId && stored.judgeId) {
  ...
  // Validate stored judgeId against current competition's judges
  useEffect(() => {
  ...
  }, [competitionId, isLoaded, loggedInJudgeId, judgeId, filteredJudges.some]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handlePlayerSelect = useCallback((player: SelectPlayer) => {

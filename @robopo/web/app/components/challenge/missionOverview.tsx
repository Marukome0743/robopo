"use client"

import {
  CheckCircleIcon,
  ChevronRightIcon,
  XMarkIcon,
} from "@heroicons/react/24/outline"
import { Bars3BottomLeftIcon } from "@heroicons/react/24/solid"
import { useEffect, useRef, useState } from "react"
import {
  getMissionParameterUnit,
  MissionString,
  type MissionValue,
  type PointState,
} from "@/app/components/course/utils"

type MissionOverviewProps = {
  missionPair: MissionValue[][]
  pointState: PointState
  nowMission: number
  isGoal: boolean
}

export function MissionOverview({
  missionPair,
  pointState,
  nowMission,
  isGoal,
}: MissionOverviewProps) {
  const [open, setOpen] = useState(false)
  const drawerRef = useRef<HTMLDivElement>(null)

  // Close on Escape
  useEffect(() => {
    if (!open) {
      return
    }
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        setOpen(false)
      }
    }
    window.addEventListener("keydown", handler)
    return () => window.removeEventListener("keydown", handler)
  }, [open])

  // Scroll current mission into view
  useEffect(() => {
    if (!open || !drawerRef.current) {
      return
    }
    const current = drawerRef.current.querySelector("[data-current]")
    current?.scrollIntoView({ block: "center", behavior: "smooth" })
  }, [open])

  const completedCount = isGoal ? missionPair.length : nowMission
  const total = missionPair.length
  const progress = total > 0 ? (completedCount / total) * 100 : 0

  return (
    <>
      {/* Toggle button */}
      <button
        type="button"
        className="btn btn-ghost btn-sm gap-1"
        onClick={() => setOpen(true)}
        aria-label="ミッション一覧を開く"
      >
        <Bars3BottomLeftIcon className="size-5" />
        <span className="font-medium text-xs">
          {completedCount}/{total}
        </span>
      </button>

      {/* Overlay + Drawer */}
      {open && (
        <div className="fixed inset-0 z-50 flex justify-end">
          {/* biome-ignore lint/a11y/noStaticElementInteractions: backdrop dismiss pattern */}
          <div
            className="absolute inset-0 bg-black/30 backdrop-blur-[2px]"
            onClick={() => setOpen(false)}
            onKeyDown={() => {}}
            role="presentation"
          />

          {/* Drawer panel */}
          <div
            ref={drawerRef}
            className="drawer-slide-in relative flex w-80 max-w-[85vw] flex-col bg-base-100 shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-base-300 border-b px-4 py-3">
              <div>
                <h3 className="font-bold text-base">ミッション一覧</h3>
                <p className="text-base-content/50 text-xs">
                  {completedCount} / {total} 完了
                </p>
              </div>
              <button
                type="button"
                className="btn btn-ghost btn-sm btn-circle"
                onClick={() => setOpen(false)}
                aria-label="閉じる"
              >
                <XMarkIcon className="size-5" />
              </button>
            </div>

            {/* Progress bar */}
            <div className="px-4 py-2">
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-base-300">
                <div
                  className="h-full rounded-full bg-primary transition-all duration-500"
                  style={{ width: `${progress}%` }}
                />
              </div>
            </div>

            {/* Mission list */}
            <div className="flex-1 overflow-y-auto px-2 pb-4">
              {missionPair.map((pair, i) => {
                const isCompleted = isGoal || i < nowMission
                const isCurrent = !isGoal && i === nowMission
                const pointEntry = pointState[i + 2]
                const isTier = Array.isArray(pointEntry)

                return (
                  <div
                    // biome-ignore lint/suspicious/noArrayIndexKey: mission pairs can have duplicate values
                    key={`mission-${i}-${String(pair[0])}`}
                    data-current={isCurrent ? "" : undefined}
                    className={`mx-1 my-1 flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors ${
                      isCurrent
                        ? "bg-primary/10 ring-1 ring-primary/30"
                        : isCompleted
                          ? "bg-success/5"
                          : "bg-base-200/50"
                    }`}
                  >
                    {/* Status icon / Number */}
                    {isCompleted ? (
                      <CheckCircleIcon className="size-6 shrink-0 text-success" />
                    ) : isCurrent ? (
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary font-bold text-primary-content text-xs">
                        {i + 1}
                      </div>
                    ) : (
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-base-300 font-medium text-base-content/40 text-xs">
                        {i + 1}
                      </div>
                    )}

                    {/* Mission info */}
                    <div className="min-w-0 flex-1">
                      <p
                        className={`font-semibold text-sm leading-tight ${
                          isCurrent
                            ? "text-primary"
                            : isCompleted
                              ? "text-base-content/60"
                              : "text-base-content/40"
                        }`}
                      >
                        {pair[0] === null ? "-" : MissionString[pair[0]]}
                        {pair[1] === null ? "" : ` ${pair[1]}`}
                        {pair[0] === null
                          ? ""
                          : getMissionParameterUnit(pair[0])}
                      </p>
                      <p
                        className={`text-xs ${
                          isCurrent
                            ? "text-accent"
                            : isCompleted
                              ? "text-success/60"
                              : "text-base-content/30"
                        }`}
                      >
                        {isTier
                          ? "段階評価"
                          : pointEntry !== null
                            ? `${pointEntry}pt`
                            : "-"}
                      </p>
                    </div>

                    {/* Current indicator */}
                    {isCurrent && (
                      <ChevronRightIcon className="size-4 shrink-0 animate-pulse text-primary" />
                    )}
                  </div>
                )
              })}
            </div>
          </div>
        </div>
      )}
    </>
  )
}

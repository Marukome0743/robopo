...
export async function DELETE(req: Request) {
  ...
  // Check if any course is linked to competitions
  for (const courseId of ids) {
    const count = await getCourseCompetitionCount(courseId)
    if (count > 0) {
      return Response.json(
        {
          success: false,
          message:
            "使用大会が0でないコースは削除できません。削除するには、先に大会一覧からコースの紐付けを解除してください。",
        },
        { status: 400 },
      )
    }
